...
def recursion_binarysearchvalues(L, v):
    return rec_binary(L, v, [], 0, len(L)-1)


def rec_binary(listValues, val, output, low, high):
    mid = (low + high) // 2
    output.append((mid, listValues[mid]))
    if listValues[mid] > val and low < high:
        return rec_binary(listValues, val, output, low, mid - 1)
    elif listValues[mid] < val and low < high:
        return rec_binary(listValues, val, output, mid + 1, high)
    return output
